...
def cnn_model_fn(features, labels, mode):
    ...
    pool3_flat = tf.reshape(pool3, [-1, 27 * 27 * 16])

    # Fully Connected Layer
    dense = tf.layers.dense(inputs=pool3_flat, units=50, activation=tf.nn.relu)
...
def main(unused_argv):
    ...
    eval_labels = np.asarray(spectrograms.test.labels, dtype=np.int32)

    # Create the Estimator
    spectrograms_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir="/tmp/spectrograms_convnet_model")
    ...
    eval_input_fn = tf.estimator.inputs.numpy_input_fn(
        ...
        num_epochs=1,
        shuffle=False)
    eval_results = spectrograms_classifier.evaluate(input_fn=eval_input_fn)
    print(eval_results)
