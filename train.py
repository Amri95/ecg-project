...
def model_fn(features, labels, mode, params):
    ...
    net = tf.layers.dense(inputs=net, name='layer_fc1',
                          units=50, activation=tf.nn.relu)

    net = tf.layers.dropout(net, rate=0.2, noise_shape=None,
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        # # trying to use sgd like in the paper
...
count = 0
while (count < 100000): # count is how many times to train the model and then evaluate
    model.train(input_fn=train_input_fn, steps=1000) # how many steps for gradient descent
